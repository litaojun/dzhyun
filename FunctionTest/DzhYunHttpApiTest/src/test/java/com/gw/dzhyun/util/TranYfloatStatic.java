...
public class TranYfloatStatic {
	public static  Yfloat yf = new Yfloat();
	private static JSONArray delJsonArr = null;
	private static JSONObject  delJson = null;
	 ...
	 */
	public TranYfloatStatic(JSONObject jsonobj,String key)
	{
	 ...
	 * ，根据接口不同，对应的key=RepDataZhiBiaoShuChu（指标计算），RepDataNewsInfoValue（新闻服务）等
	 */
	public TranYfloatStatic(String jsonstr,String key)
	{
		this.delJson = JSON.parseObject(jsonstr);
		this.delJsonArr = delJson.getJSONObject("Data").getJSONArray(key);
	}
	
	public TranYfloatStatic()
	{
	}
	public static void InitJsonArrByStr(String jsonstr,String key)
	{
		delJson = JSON.parseObject(jsonstr);
		delJsonArr = delJson.getJSONObject("Data").getJSONArray(key);
	}
	public static void InitJsonArrByJson(JSONObject jsonobj,String key)
	{
		delJson = jsonobj;
		delJsonArr = jsonobj.getJSONObject("Data").getJSONArray(key);
	}
	 ...
	 */
	public TranYfloatStatic(String jsonstr)
	{
		this.delJson = JSON.parseObject(jsonstr);
		String key = this.getKeyByLength(this.delJson);
		this.delJsonArr = delJson.getJSONObject("Data").getJSONArray(key);
	}
	public static String getKeyByLength(JSONObject jsonObj)
	{
	...
	public static void dealNormalJsonObj(Object obj,String key,JSONObject jsb)
	{
		...
		else
		{
			...
			YfloatObj yoj = yf.UnmakeValue(a);
			if(yoj.getdp() == 0)
			{
	 ...
	 * JSONArray:处理的JSON数组
	 */
	public static void dealNormalJsonArrObj(Object obj,int num,JSONArray jsbArr,String key)
	{
	...
	public static void init(String jsonstr,String key)
	{
		if(sign == 0)
		{
			delJson = JSON.parseObject(jsonstr);
	...
	public static JSONObject startTrans(String jsonstr,String key)
	{
		init(jsonstr, key);
		dealJsonArray();
		sign=0;
	...
	public static JSONObject startTrans2(String jsonstr)
	{
		
		if(sign == 0)
		{
			delJson = JSON.parseObject(jsonstr);
			String key = getKeyByLength(delJson);
			delJsonArr = delJson.getJSONObject("Data").getJSONArray(key);
			sign =1;
    ...
    * 处理json数组
    */
	public static JSONObject dealJsonArray()
	{
		
		for(int i=0;i<delJsonArr.size();i++)
		{
			JSONObject temp = delJsonArr.getJSONObject(i);
			tranStartJson(temp);
		}
		return delJson;
	}
	 ...
	 */
	public static void tranStartJson(JSONObject jsb)
	{
...
//		System.out.println(jsonUAResponse);
		String[][] a = new String[][]{//{"RepDataNewsInfoValue","http://10.15.144.80/news/get?obj=SH600000&type=1"},
				                                  ...
				                                  {"RepDataJianPanBaoShuChu","http://10.15.144.80/news/get?obj=SH600000&type=1"}
				                                  };
		try {
...
//			    JSONObject jsonUAResponse = aa.dealJsonArray();
			    ...
			    JSONObject  jsonUAResponse = TranYfloatStatic.startTrans2(ret);
